...
void setup()
{
  ...
  //-------------------------My Added Code---------------------------------------
  initQuadrant = 0; // Initialize to zero
  inTouch = false; // Set flag to false (assume no one is touching the board yet)
  validTouch = true; // Default assumption: valid
  startTime = 0;
  //-----------------------------------------------------------------------------
...
{  
  ...
  { 
    ...
    // First time through, determine the initial touch info
    if (!inTouch) {
      // First mark the time of touch
      startTime = millis(); 
      inTouch = true; // Raise this flag

      if (!touchData.hovering && !Pinnacle_zIdlePacket(&touchData)) {
        validTouch = true; // Tentatively mark the touch as valid
      }

      // Record the initial quadrant touched
      if (touchData.xValue < PINNACLE_XMAX/2) {
        if ((touchData.yValue < PINNACLE_YMAX/2) && (touchData.yValue != 0)) {
          initQuadrant = 0;
        }
        else if (touchData.yValue > PINNACLE_YMAX/2){
          initQuadrant = 3;
        }
      }
      else {
    ...
    // Continuously check for violations of our "valid touch" parameters

    // If the time ever exceeds the max, the touch is invalid
    if (millis() - startTime > TAP_TIMEOUT_MSEC) {
      validTouch = false;
    }

    // If the quadrant ever changes, the touch is invalid
    if (touchData.xValue < PINNACLE_XMAX/2) {
      if ((touchData.yValue < PINNACLE_YMAX/2) && (touchData.yValue != 0)) {
        if (initQuadrant != 0) {
          validTouch = false;
    ...
    // Wait until liftoff before reporting anything
    if(Pinnacle_zIdlePacket(&touchData) && inTouch)
    {
      inTouch = false; // Lower the flag
      
      // If the touch was valid, print the quadrant
      if (validTouch) {
        Serial.print("Q");
        Serial.println(initQuadrant);
        validTouch = false; // Lower the flag
      }
    }
  }
